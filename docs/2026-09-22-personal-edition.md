...
- Release build/signature/install passed. The installed bar and dashboard showed four distinct live account readings; a manual refresh updated all four persisted timestamps while preserving separate results. No reconnection was needed.
- Runtime stability remains unresolved: the user reported another Mac-wide freeze during the dashboard verification. The personal app was closed, and process inspection confirmed no Codenotch, compiler or test process remained. This is not a verified resolution of the freeze; keep the PR in draft pending a controlled responsiveness investigation.
